...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
 ...
 if ($nome !== "" && $localizacao !== "" && $mensagem !== "") {
    $stmt = mysqli_prepare(
    $conexao,
    "INSERT INTO livro (nome, localizacao, mensagem, data) VALUES (?, ?, ?, NOW())" );
    mysqli_stmt_bind_param($stmt, "sss", $nome, $localizacao, $mensagem);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: mostrar.php");
    exit;
 } else { $err = "Preencha todos os campos!"; }
...
<?php } ?>
    <form method="post" action="assinar.php">
        <table>
            <tr><!--  style="display: block; background-color: #ffffff; color: #000000; font-weight: normal;" -->
                <td width="150">Nome:</td>
                <td><input type="text" size="15" name="nome" 
                value="<?php echo htmlspecialchars($_POST["nome"] ?? "",ENT_QUOTES, "UTF-8"); ?>"></td>
            </tr>
            <tr>
                <td width="150">Localização:</td>
                <td>
                <input type="text" size="15" name="localizacao" 
                value="<?php echo htmlspecialchars($_POST["localizacao"] ?? "", ENT_QUOTES, "UTF-8"); ?>">
                </td>
            </tr>
            <tr>
                <td colspan="2">
                <textarea rows="10" name="mensagem" placeholder="Digite aqui sua mensagem!">
